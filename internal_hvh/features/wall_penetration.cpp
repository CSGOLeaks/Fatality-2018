...
bool penetration::get_damage( C_CSPlayer* shooter, C_CSPlayer* target, Vector end, float* damage, CCSWeaponData* custom_weapon, Vector* start_pos, matrix3x4_t* matrix )
{
	...
	*damage = 0.f;
	...
	if ( simulate_bullet_fire( shooter, target, weapon, data, end ) )
	{
...
bool penetration::simulate_bullet_fire( C_CSPlayer* shooter, C_CSPlayer* target, C_BaseCombatWeapon* weapon, FireBulletData& data, const Vector& endpos )
{
	...
	while ( data.penetrate_count > 0 && data.current_damage >= 1.f )
	{
		...
		auto weaponData = weapon->get_wpn_data();

		if ( !handle_bullet_penetration(weaponData, data.enter_trace, data.src, data.direction, data.penetrate_count, data.current_damage, weaponData->flPenetration, true, ff_damage_reduction_bullets, ff_damage_bullet_penetration, target ? true : false ) )
			break;
	}
