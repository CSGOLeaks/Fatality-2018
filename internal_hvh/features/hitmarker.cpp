...
void hitmarker::listener( IGameEvent * game_event )
{
	...
	if (!vars::misc.hit_effect.get<bool>()) 
		player->m_flHealthShotBoostExpirationTime() = g_pGlobals->curtime + 0.5f;

	if (vars::misc.kill_effect.get<bool>())
		player->m_flHealthShotBoostExpirationTime() = g_pGlobals->curtime + 1.f;
